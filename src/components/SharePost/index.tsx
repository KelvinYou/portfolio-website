...
type ShareItemNameType = 'linkedin' | 'whatsapp' | 'facebook';

type ShareItem = {
  name: ShareItemNameType,
  label: string,
  url?: string,
  text?: string,
}

const ICON_MAP: Record<ShareItemName, JSX.Element> = {
  'linkedin': <svg
    width="20"
    height="20"
    viewBox="0 0 16 16"
  ...
  'whatsapp': <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" 
    width="24" height="24" 
    viewBox="0 0 48 48">
  ...
  'facebook': <svg width="9" height="18" viewBox="0 0 9 18" className="fill-current">
...
const SharePost: React.FC<SharePostProps> = ({
  ...
  const getShareItemUrl = (shareItem: ShareItem): string | null => {
    const { name, url, text } = shareItem;
    const encodedUrl = encodeURIComponent(url || defaultUrl);

    switch (name) {
      case ShareItemName.LinkedIn:
        return `https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}`;
      case ShareItemName.WhatsApp:
        return `https://wa.me/?text=${text || defaultText} \n ${defaultUrl}`;
      case ShareItemName.Facebook:
  ...
  const shareToSocialMedia = (shareItem: ShareItem) => {
    const shareUrl = getShareItemUrl(shareItem);
    if (shareUrl) {
      window.open(shareUrl, '_blank');
    } else {
      console.error('Unsupported share item:', shareItem);
    }
  };
  ...
  return (
    <div className="mb-5">
      ...
      <div className="flex items-center gap-2">
        {shareItems.map((shareItem, index: number) => {
          return (
              <ShareButton
                key={index}
                label={shareItem.label}
                icon={ICON_MAP[shareItem.name]}
                onClick={() => shareToSocialMedia(shareItem)}
              />
            )
        })}
      </div>
